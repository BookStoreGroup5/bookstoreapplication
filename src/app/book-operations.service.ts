...
export class BookOperationsService {
  ...
  getBookByTitleEndPoint=this.baseURL+'/book/title';
  ...
  bookByTitle(title:string):Observable<BookDetailedDTO[]>{
    console.log("inside get book by title" +this.getBookByTitleEndPoint);
    this.getBookByTitleEndPoint=this.getBookByTitleEndPoint+'/'+title;
    return this.http.get<BookDetailedDTO[]>(`${this.getBookByTitleEndPoint}`);
  }
